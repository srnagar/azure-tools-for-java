            ...
            public boolean collect(@Nonnull PsiElement psiElement, @Nonnull Editor editor, @Nonnull InlayHintsSink inlayHintsSink) {
                if(psiElement instanceof PsiNewExpression) {
                    ...
                    if (newExpression.getClassReference() != null && newExpression.getClassReference().getCanonicalText() != null
                        ...
                        final InlayPresentation iconPresentation = new PresentationFactory(editor).icon(ICON);
                        final InlayPresentationFactory.ClickListener azureClickListener = (mouseEvent, point) -> {
                            AzureResourceListWindow.showPopup(psiElement.getProject(), psiElement.getText(), mouseEvent.getComponent());
                        };
                        final OnClickPresentation azureClickPresentation = new OnClickPresentation(iconPresentation, azureClickListener);
                        final WithCursorOnHoverPresentation presentation = new WithCursorOnHoverPresentation(azureClickPresentation, HAND_CURSOR, editor);

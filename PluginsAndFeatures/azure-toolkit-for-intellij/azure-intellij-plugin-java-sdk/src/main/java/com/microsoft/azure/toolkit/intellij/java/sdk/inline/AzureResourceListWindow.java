package com.microsoft.azure.toolkit.intellij.java.sdk.inline;
...
public class AzureResourceListWindow {
    ...
    private static final Vector<String> COLUMNS = new Vector<>(Arrays.asList("Select", "Resource name", "Resource Type", "Resource Group", "Portal"));
    ...
    public static void showPopup(Project project, String title, Component parent) {
        ...
        final Vector<Vector<Object>> items = new Vector<>();
        ...
        resourcesServiceSubscription
                ...
                .forEach(resource -> {
                    ...
                    columns.add(Boolean.FALSE); // radio button selection state
        ...
        DefaultTableModel tableModel = new DefaultTableModel(items, COLUMNS) {
            ...
            public boolean isCellEditable(int row, int column) {
                return column == 0; // Only radio button column is editable
            }
            @Override
        ...
        // Custom renderer for radio button column
        ...
        // Only allow one radio button selected at a time
        ...
        // When a row is selected, update radio button
        ...
        // Set hyperlink renderer for last column (now index 4)
                ...
                ApplicationManager.getApplication().runWriteAction(() -> {
                    if (title.contains("Configuration")) {
                        ...
                        String existingResource = "resource storage 'Microsoft.Storage/storageAccounts@2023-04-01' existing = {\n" +
                                "  name: '" + table.getStringAt(table.getSelectedRow(), 1) + "'\n" +
                                "}";

                        String referenceModule = EXISTING_STORAGE_MODULE.replace("rg.name", "'" + table.getStringAt(table.getSelectedRow(), 3) + "'");
                        ...
                        String referenceModule = Matcher.quoteReplacement(EXISTING_OPENAI_MODULE)
                                .replace("rg.name", "'" + table.getStringAt(table.getSelectedRow(), 3) + "'") + "\n\noutput";
